...
def moving_crop(video):
    ...
    """
    script = "swaprect=720:480:0:0:400:400:enable='between(n,0,100)'," \
    ...
    with open('filter_script.txt', 'w') as file:
        file.write(script)
    cmd = f"{ffmpeg} -y -v quiet -ss 0 -t 30 -i {video.getfile()} -filter_complex_script filter_script.txt -acodec copy " \
